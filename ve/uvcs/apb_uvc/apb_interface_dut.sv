interface apb_interface_dut (input logic pclk, input logic rst_n);
   ...
   // ------------------------------------------------------------------ //
   // Additional APB Protocol Assertions (AMBA APB Spec compliant)
   // ------------------------------------------------------------------ //

   // PENABLE should only be high when PSEL is high (ACCESS phase)
   property penable_only_with_psel;
      @(posedge pclk) disable iff(rst_n === 1'b0)
         penable |-> psel;
   endproperty

   // PSEL should not be deasserted while PENABLE is high (during ACCESS phase)
   property psel_stable_during_access;
      @(posedge pclk) disable iff(rst_n === 1'b0)
         (psel && penable && !pready) |=> psel;
   endproperty

   // PREADY should eventually be asserted (no infinite wait states)
   property pready_eventually_asserted;
      @(posedge pclk) disable iff(rst_n === 1'b0)
         (psel && penable) |-> ##[0:100] pready;  // Reasonable timeout for simulation
   endproperty

   // PSLVERR is only valid when PSEL, PENABLE, and PREADY are all HIGH
   property pslverr_only_valid_with_complete_transfer;
      @(posedge pclk) disable iff(rst_n === 1'b0)
         pslverr |-> (psel && penable && pready);
   endproperty

   // Transfer completion: PENABLE deasserted after PREADY asserted
   property penable_deassert_after_transfer;
      @(posedge pclk) disable iff(rst_n === 1'b0)
         (psel && penable && pready) |=> !penable;
   endproperty

   // PADDR stable throughout transfer (SETUP and ACCESS phases)
   property paddr_stable_throughout_transfer;
      @(posedge pclk) disable iff(rst_n === 1'b0)
         (psel && !penable) |-> ##1 ($stable(paddr) throughout (psel && penable)[->1] ##0 pready[->1]);
   endproperty

   // PWRITE stable throughout transfer
   property pwrite_stable_throughout_transfer;
      @(posedge pclk) disable iff(rst_n === 1'b0)
         (psel && !penable) |-> ##1 ($stable(pwrite) throughout (psel && penable)[->1] ##0 pready[->1]);
   endproperty

   // PWDATA stable during write transfers in ACCESS phase
   property pwdata_stable_during_write_access;
      @(posedge pclk) disable iff(rst_n === 1'b0)
         (psel && penable && pwrite && !pready) |=> $stable(pwdata);
   endproperty

   // State transitions: SETUP (PSEL=1, PENABLE=0) to ACCESS (PSEL=1, PENABLE=1)
   property setup_to_access_transition;
      @(posedge pclk) disable iff(rst_n === 1'b0)
         (psel && !penable) |=> (psel && penable);
   endproperty

   penable_only_with_psel_a: assert property (penable_only_with_psel)
      else $error("[%0t][APB_INTF] ERROR: penable asserted without psel", $time);

   psel_stable_during_access_a: assert property (psel_stable_during_access)
      else $error("[%0t][APB_INTF] ERROR: psel deasserted during access phase", $time);

   pready_eventually_asserted_a: assert property (pready_eventually_asserted)
      else $error("[%0t][APB_INTF] ERROR: pready never asserted within 100 cycles", $time);

   pslverr_only_valid_with_complete_transfer_a: assert property (pslverr_only_valid_with_complete_transfer)
      else $error("[%0t][APB_INTF] ERROR: pslverr asserted outside valid transfer completion", $time);

   penable_deassert_after_transfer_a: assert property (penable_deassert_after_transfer)
      else $error("[%0t][APB_INTF] ERROR: penable not deasserted after transfer completion", $time);

   paddr_stable_throughout_transfer_a: assert property (paddr_stable_throughout_transfer)
      else $error("[%0t][APB_INTF] ERROR: paddr changed during transfer", $time);

   pwrite_stable_throughout_transfer_a: assert property (pwrite_stable_throughout_transfer)
      else $error("[%0t][APB_INTF] ERROR: pwrite changed during transfer", $time);

   pwdata_stable_during_write_access_a: assert property (pwdata_stable_during_write_access)
      else $error("[%0t][APB_INTF] ERROR: pwdata changed during write access", $time);

   setup_to_access_transition_a: assert property (setup_to_access_transition)
      else $error("[%0t][APB_INTF] ERROR: invalid transition from SETUP to ACCESS", $time);

   // ------------------------------------------------------------------ //
   // Coverage
   // ------------------------------------------------------------------ //

   // Cover successful read/write transfers
   cover_read_transfer: cover property (@(posedge pclk) disable iff(!rst_n)
      (psel && !penable && !pwrite) ##1 (psel && penable && !pwrite) ##[0:$] (pready && !pslverr));

   cover_write_transfer: cover property (@(posedge pclk) disable iff(!rst_n)
      (psel && !penable && pwrite) ##1 (psel && penable && pwrite) ##[0:$] (pready && !pslverr));

   cover_error_transfer: cover property (@(posedge pclk) disable iff(!rst_n)
