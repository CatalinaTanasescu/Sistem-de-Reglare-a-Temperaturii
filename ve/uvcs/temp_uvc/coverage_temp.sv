class coverage_temp extends uvm_component;
  ...
  covergroup cg_temp_item with function sample(tranzactie_temp item);
    ...
    TEMP_VALID: coverpoint item.temp_valid {
      bins valid   = {1};
      bins invalid = {0};
    }
  ...
  virtual function void write_temp(tranzactie_temp t);
    ...
    `uvm_info("TEMP_COV", $sformatf("Sampled: Delay=%0d | Temp=%0d | Valid=%0b | Heater=%0b | Cooler=%0b | Coverage: %0.2f%%",
              t.delay, t.temp_now, t.temp_valid, t.heater_on, t.cooler_on, cg_temp_item.get_inst_coverage()), UVM_LOW)
